...
#if 1  //add by hzb for remove dev from ontim_debug
     struct kobject * debug_kobj;
	 struct attribute_group * attr_group;
#endif
	 struct kobject *ontim_debu_kobj;
};
...
#if 0  //deleat by hzb for remove dev from ontim_debug
...
//    ontim_debug->dbg_trace_level=0;
...
#if 1  //modfiy by hzb for remove dev from ontim_debug
    ontim_core_dbg_item->debug_kobj = ontim_dev_debu_kobj;
    kobj_attr=kzalloc(sizeof(struct kobj_attribute)*(ontim_debug->dev_attr_array_len+1),GFP_KERNEL);
    memset(kobj_attr,0,sizeof(struct kobj_attribute)*(ontim_debug->dev_attr_array_len+1));
    ontim_core_dbg_item->kobj_attr = kobj_attr;
#else
    kobj_attr=alloc_attribute_from_kobj_attribute_arrary(ontim_debug->dev_attr_array_len+1);
#endif
    if(kobj_attr==NULL)
       goto malloc_fail2;//modfiy by hzb for remove dev from ontim_debug

    properties_attrs=kzalloc(sizeof(struct attribute *)*(ontim_debug->dev_attr_array_len+2),GFP_KERNEL);
    if(properties_attrs==NULL)
       goto malloc_fail3;//modfiy by hzb for remove dev from ontim_debug
    ...
    if(properties_attr_group==NULL)
       goto malloc_fail4;//modfiy by hzb for remove dev from ontim_debug
	...
	printk(KERN_ERR "failed to ontim debug,dev_name=%s\n",ontim_debug->dev_name);
        goto  malloc_fail5;//modfiy by hzb for remove dev from ontim_debug
    }
    list_add_tail(&ontim_core_dbg_item->node, &ontim_core_debug_list);

#if 1  //add by hzb for remove dev from ontim_debug
    ontim_core_dbg_item->kobj_attr = kobj_attr;
    ontim_core_dbg_item->attr_group = properties_attr_group;
#endif
	ontim_core_dbg_item->ontim_debu_kobj=ontim_dev_debu_kobj;
    ...
    return 0;
//+modfiy by hzb for remove dev from ontim_debug
malloc_fail5:
...
malloc_fail3:
    kfree(kobj_attr);
malloc_fail2:
    kobject_put(ontim_dev_debu_kobj);
malloc_fail1:
//-modfiy by hzb for remove dev from ontim_debug
...
#if 1  //add by hzb for remove dev from ontim_debug
int unregester_and_deinit_ontim_debug(struct ontim_debug *ontim_debug)
{
    struct ontim_core_debug *ontim_core_dbg_item;
    ...
    if (ontim_debug == NULL)
        return -1;
    if (ontim_debug->dev_name==NULL)
        return -1;

    list_for_each_entry(ontim_core_dbg_item, &ontim_core_debug_list, node) {
        if (!strncmp(ontim_core_dbg_item->name,ontim_debug->dev_name,NAME_LEN)) {
             //printk(KERN_ERR "ontim_core_dbg_item->name=%s\n",ontim_core_dbg_item->name);
             //printk(KERN_ERR "*ontim_debug->dev_name=%s\n",(*ontim_debug)->dev_name);
	     break;
        }
    }
    if (&(ontim_core_dbg_item->node) == &ontim_core_debug_list)
		return -1;
    list_del(&ontim_core_dbg_item->node);

    if (ontim_core_dbg_item->attr_group)
    {
		if (ontim_core_dbg_item->debug_kobj)
			sysfs_remove_group(ontim_core_dbg_item->debug_kobj, ontim_core_dbg_item->attr_group);
        if (ontim_core_dbg_item->attr_group->attrs)
			kfree(ontim_core_dbg_item->attr_group->attrs);
		kfree(ontim_core_dbg_item->attr_group);
    }
    if (ontim_core_dbg_item->kobj_attr)
        kfree(ontim_core_dbg_item->kobj_attr);

    if (ontim_core_dbg_item->debug_kobj)
        kobject_put(ontim_core_dbg_item->ontim_debu_kobj);
    kfree(ontim_core_dbg_item);
	return 0;
}
#endif
...
EXPORT_SYMBOL_GPL(unregester_and_deinit_ontim_debug);
